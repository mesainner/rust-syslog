...
pub fn unix<U: Display, F: Clone + LogFormat<U>>(
  formatter: F,
...
fn unix_connect<P: AsRef<Path>, U: Display, F: LogFormat<U>>(
  formatter: F,
  path: P,
) -> Result<Logger<LoggerBackend, U, F>> {
...
pub fn udp<T: ToSocketAddrs, U: Display, F: LogFormat<U>>(
  formatter: F,
  local: T,
  server: T,
) -> Result<Logger<LoggerBackend, U, F>> {
...
pub fn tcp<T: ToSocketAddrs, U: Display, F: LogFormat<U>>(
  formatter: F,
  server: T,
) -> Result<Logger<LoggerBackend, U, F>> {
...
pub fn init(
  ...
  let backend = unix(formatter.clone())
    .map(|logger: Logger<LoggerBackend, String, Formatter3164>| logger.backend)
    .or_else(|_| {
